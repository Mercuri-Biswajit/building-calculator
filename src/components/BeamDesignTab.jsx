    ...
    <div className="calc-result-card">
      ...
      <div className="calc-card">
        ...
        <div className="calc-card-section">
          ...
          <div className="calc-grid-3">
            <InputField
              name="Mu"
              label="Factored Bending Moment, Mu (kNm) *"
              hint="Design moment at critical section"
              placeholder="150"
              value={inputs.Mu}
            ...
            <InputField
              name="Vu"
              label="Factored Shear Force, Vu (kN) *"
              hint="Design shear at support"
              placeholder="80"
              value={inputs.Vu}
              onChange={onInputChange}
              step="0.1"
        ...
        <div className="calc-card-section">
          ...
          <div className="calc-grid-3">
            ...
            <InputField
              name="span"
              label="Span, L (mm)"
              hint="For deflection check (optional)"
              placeholder="6000"
              value={inputs.span}
              onChange={onInputChange}
            />
        ...
        <div className="calc-card-section">
          ...
          <div className="calc-grid-3">
            ...
            <InputField
              ...
              hint="Mild: 25, Moderate: 30, Severe: 45"
      ...
      {results && !results.error && (
        <>
          ...
          {results.shearDesign && (
            <ShearDetails shearDesign={results.shearDesign} />
          )}
          {results.deflectionCheck && (
            <DeflectionCheck deflectionCheck={results.deflectionCheck} />
          )}
          {results.developmentLength && (
            <AnchorageDetails developmentLength={results.developmentLength} />
          )}
    ...
    <div className="calc-struct-section">
      ...
      <div className="calc-struct-grid">
        <div
          ...
          style={{
            borderColor: designType === "singly" ? "#3B82F6" : "#F59E0B",
          }}
        >
        ...
        <div className="calc-struct-card">
          ...
          <div className="calc-struct-value">
            {safeGet(results, "summary.tension")}
          </div>
          <div className="calc-struct-sub">Main reinforcement (bottom)</div>
        </div>

        {safeGet(results, "summary.compression") !== "Not required" &&
          safeGet(results, "summary.compression") !== "N/A" && (
            <div className="calc-struct-card">
              <div className="calc-struct-icon">🔧</div>
        ...
        <div className="calc-struct-card">
          ...
          <div className="calc-struct-value">
            {safeGet(results, "summary.stirrups")}
          </div>
...
function FlexuralDetails({ results, inputs }) {
  ...
  const barOptions =
    flexuralDesign?.barOptions || flexuralDesign?.tensionBarOptions;
    ...
    <div className="calc-card">
      ...
      <div className="calc-detail-grid">
        <DetailItem
          ...
          value={`${safeFormat(results.inputs.d, 0)} mm`}
        />
        <DetailItem
          label="Effective Cover (d')"
          value={`${safeFormat(results.inputs.d_prime, 0)} mm`}
        />
        <DetailItem
          label="Limiting Moment (Mu,lim)"
          value={`${safeFormat(results.limiting?.Mu_lim, 2)} kNm`}
        />
        <DetailItem
          label="xu,max / d"
          value={safeFormat(results.limiting?.xu_max_by_d, 3)}
        />
      </div>
      ...
      {flexuralDesign && (
        <div style={{ marginTop: "20px" }}>
          ...
          <div className="calc-detail-grid">
            <DetailItem
              label="Ast Required"
              value={`${safeFormat(flexuralDesign.Ast_required, 0)} mm²`}
            />
            <DetailItem
              label="Ast Minimum"
              value={`${safeFormat(flexuralDesign.Ast_min, 0)} mm²`}
            />
            <DetailItem
              label="Ast Provided"
            ...
            <DetailItem
              label="Reinforcement %"
              value={`${safeFormat(flexuralDesign.pt_provided, 2)}%`}
            />
          </div>
        </div>
      )}
                  ...
                  >
                    ...
                    <td>
                      <span
                        style={{
                          color:
                            opt.spacing >= 75 && opt.spacing <= 300
                              ? "#10B981"
                              : "#F59E0B",
                          fontSize: "18px",
                        }}
      ...
      {flexuralDesign?.designType === "doubly" &&
        flexuralDesign?.compressionBarOptions && (
          <div style={{ marginTop: "20px" }}>
            <h5 className="calc-section-label">Compression Steel Options</h5>
            <div className="calc-detail-grid">
              <DetailItem
                label="Asc Required"
                value={`${safeFormat(flexuralDesign.Asc_required, 0)} mm²`}
              />
              <DetailItem
                label="Additional Moment"
                value={`${safeFormat(flexuralDesign.Mu2, 2)} kNm`}
              />
            </div>
                ...
                <tbody>
                  {flexuralDesign.compressionBarOptions.map((opt, idx) => (
                    <tr
                      key={idx}
                      style={{
                        backgroundColor: idx === 0 ? "#FFF7ED" : "transparent",
                        fontWeight: idx === 0 ? "bold" : "normal",
                      }}
                    >
                      <td>
    ...
    <div className="calc-card">
      ...
      <div className="calc-detail-grid">
        <DetailItem
          ...
          value={`${safeFormat(shearDesign.tau_v, 3)} MPa`}
        />
        <DetailItem
          label="Design Shear Strength (τc)"
          value={`${safeFormat(shearDesign.tau_c, 3)} MPa`}
        />
        <DetailItem
          label="Max Shear Stress (τc,max)"
          value={`${safeFormat(shearDesign.tau_c_max, 2)} MPa`}
        />
        <DetailItem
          label="Design Status"
          value={safeGet(shearDesign, "status", "UNKNOWN")}
          valueStyle={{
            color: shearDesign.status === "FAIL" ? "#EF4444" : "#10B981",
          }}
        />
      </div>
      ...
      {shearDesign.status === "FAIL" && (
        <div className="calc-alert calc-alert-error">
          ...
          {safeGet(shearDesign, "message", "Check section dimensions")}
        </div>
      )}

      {shearDesign.stirrupOptions && (
        <StirrupTable options={shearDesign.stirrupOptions} showRecommended />
      )}
      {shearDesign.stirrupDetails && (
        <>
          <h5 className="calc-section-label" style={{ marginTop: "20px" }}>
...
function DeflectionCheck({ deflectionCheck }) {
  return (
    <div className="calc-card">
      ...
      <div className="calc-detail-grid">
        <DetailItem
          label="Actual L/d Ratio"
          value={safeFormat(deflectionCheck.actual_ratio, 2)}
        />
        <DetailItem
          label="Allowable L/d Ratio"
          value={safeFormat(deflectionCheck.allowable_ratio, 2)}
        />
        <DetailItem
          label="Status"
          value={safeGet(deflectionCheck, "status", "N/A")}
          valueStyle={{
            color: deflectionCheck.status === "OK" ? "#10B981" : "#EF4444",
          }}
        />
        <DetailItem
          label="Message"
          value={safeGet(deflectionCheck, "message", "N/A")}
    ...
    >
      ...
      <ul style={{ paddingLeft: "20px", lineHeight: "1.8" }}>
        ...
        <li>
          ...
          {inputs.cover || "N/A"}mm. Verify with exposure condition
          requirements.
        </li>
        <li>
          <strong>Bar Spacing:</strong> Maintain minimum clear spacing ≥ bar
          diameter or ≥ aggregate size + 5mm
        </li>
        <li>
          <strong>Curtailment:</strong> Ensure adequate development length at
          supports and bend points
        </li>
        <li>
        ...
        {results.flexuralDesign?.designType === "doubly" && (
          <li>
            <strong>Compression Steel:</strong> Provide lateral ties to prevent
